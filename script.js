...
/* === TRUE FINAL MARQUEE FORCE FIX: JavaScript-driven infinite auto-scroll === */
(function(){
  const tracks = ['trendingMarquee','newArrivalsMarquee'];
  const rafMap = new WeakMap();

  function stop(track){
    const old = rafMap.get(track);
    if (old) cancelAnimationFrame(old);
    rafMap.delete(track);
  }

  function hasProducts(track){
    return track && track.children && track.children.length && !/No products listed yet/i.test(track.textContent || '');
  }

  function startTrack(track){
    if (!hasProducts(track)) return;
    stop(track);

    const baseHTML = track.innerHTML;
    if (!baseHTML.trim()) return;

    // Four identical groups guarantee a seamless loop even with only one or two products.
    ...
    let x = 0;
    let last = performance.now();
    const speed = window.innerWidth <= 760 ? 0.42 : 0.58; // pixels per ms, right-to-left

    function measure(){
      loopWidth = Math.max(0, track.scrollWidth / 4);
    }
    measure();

    function tick(now){
      if (!document.body.contains(track)) { stop(track); return; }
      if (!loopWidth || loopWidth < 20) measure();
      const dt = Math.min(64, now - last);
      last = now;
      if (loopWidth > 20) {
        x = (x + speed * dt) % loopWidth;
        track.style.transform = `translate3d(${-x}px,0,0)`;
      }
      rafMap.set(track, requestAnimationFrame(tick));
    }
    rafMap.set(track, requestAnimationFrame(tick));
  }
  ...
  // Let the normal product renderer load cloud products first, then force the motion.
  const previousRenderHomeSections = window.renderHomeSections;
  window.renderHomeSections = function(){
    if (typeof previousRenderHomeSections === 'function') previousRenderHomeSections.apply(this, arguments);
    setTimeout(hardStart, 80);
    setTimeout(hardStart, 500);
  };

  function boot(){
    if (!(document.getElementById('trendingMarquee') || document.getElementById('newArrivalsMarquee'))) return;
    try {
      if (typeof previousRenderHomeSections === 'function') previousRenderHomeSections();
    } catch(e) {}
    setTimeout(hardStart, 120);
    setTimeout(hardStart, 700);
    setTimeout(hardStart, 1600);
  }
  ...
  window.addEventListener('resize', () => setTimeout(boot, 200));
})();
/* === END TRUE FINAL MARQUEE FORCE FIX === */

/* === PROFESSIONAL CHECKOUT FINAL PATCH: saved addresses + $7 Aramex delivery === */
(function(){
  const DELIVERY_FEE = 7;
  function readJSON(key, fallback){try{return JSON.parse(localStorage.getItem(key)||JSON.stringify(fallback));}catch(e){return fallback;}}
  function writeJSON(key,val){localStorage.setItem(key, JSON.stringify(val));}
  function normEmail(v){return String(v||'').trim().toLowerCase();}
  function currentEmail(){try{return normEmail((JSON.parse(localStorage.getItem('nitaUser')||'null')||{}).email);}catch(e){return ''}}
  function userRecord(){const email=currentEmail(); if(!email)return null; const users=readJSON('nitaUsersByEmail',{}); return users[email]||null;}
  function setUserRecord(user){if(!user||!user.email)return; const users=readJSON('nitaUsersByEmail',{}); users[normEmail(user.email)]={...user,email:normEmail(user.email)}; writeJSON('nitaUsersByEmail',users); localStorage.setItem('nitaUser',JSON.stringify(users[normEmail(user.email)])); if(typeof saveSharedKeyNow==='function') saveSharedKeyNow('nitaUsersByEmail',users).catch(()=>{});}
  function cleanAddr(a, idx){if(!a)return null; return {label:a.label||a.addressLabel||a.name||a.title|| (idx===0?'Home':'Address '+(idx+1)), firstName:a.firstName||'', lastName:a.lastName||'', email:a.email||'', phone:a.phone||'', city:a.city||'', street:a.street||a.address||'', building:a.building||'', floor:a.floor||'', apartment:a.apartment||'', postal:a.postal||'', landmark:a.landmark||'', preferredTime:a.preferredTime||'', notes:a.notes||''};}
  function savedAddresses(){
    const user=userRecord(); const list=[];
    if(user){
      (user.addresses||[]).forEach((a,i)=>{const c=cleanAddr(a,i); if(c)list.push(c)});
      const d=cleanAddr(user.defaultAddress, list.length); if(d && (d.city||d.street||d.building)) list.unshift(d);
    }
    const local=cleanAddr(readJSON('nitaSavedAddress',null), list.length); if(local && (local.city||local.street||local.building)) list.push(local);
    const seen=new Set(); return list.filter(a=>{const k=[a.label,a.city,a.street,a.building,a.floor,a.apartment].join('|').toLowerCase(); if(seen.has(k))return false; seen.add(k); return true;});
  }
  function fillAddress(a){
    const form=document.getElementById('checkoutForm'); if(!form||!a)return;
    const u=userRecord()||{}; const full=(String(u.firstName||'')+' '+String(u.lastName||'')).trim();
    const map={firstName:a.firstName||u.firstName||full.split(' ')[0]||'',lastName:a.lastName||u.lastName||full.split(' ').slice(1).join(' ')||'',phone:a.phone||u.phone||'',city:a.city,street:a.street,building:a.building,floor:a.floor,apartment:a.apartment,postal:a.postal,landmark:a.landmark,preferredTime:a.preferredTime,notes:a.notes,addressLabel:a.label};
    Object.entries(map).forEach(([name,val])=>{const el=form.elements[name]; if(el)el.value=val||''});
  }
  window.toggleSavedAddressDetails=function(i){const card=document.querySelector(`.saved-address-card[data-index="${i}"]`); if(card) card.classList.toggle('open');}
  window.selectSavedAddress=function(i){const addrs=savedAddresses(); const addr=addrs[i]; document.querySelectorAll('.saved-address-card').forEach(c=>c.classList.remove('selected')); const card=document.querySelector(`.saved-address-card[data-index="${i}"]`); if(card)card.classList.add('selected'); const hidden=document.querySelector('[name="selectedAddressIndex"]'); if(hidden)hidden.value=i; fillAddress(addr); const formWrap=document.getElementById('newAddressForm'); if(formWrap) formWrap.classList.add('collapsed'); window.renderCheckoutSummary&&window.renderCheckoutSummary();}
  window.showNewCheckoutAddress=function(force){const formWrap=document.getElementById('newAddressForm'); if(!formWrap)return; formWrap.classList.toggle('collapsed', force===false); if(force===true) {formWrap.classList.remove('collapsed'); const hidden=document.querySelector('[name="selectedAddressIndex"]'); if(hidden)hidden.value=''; document.querySelectorAll('.saved-address-card').forEach(c=>c.classList.remove('selected'));}}
  window.setupCheckoutAddresses=function(){
    const area=document.getElementById('savedAddressArea'); const formWrap=document.getElementById('newAddressForm'); if(!area||!formWrap)return;
    const addrs=savedAddresses(); const user=userRecord(); const emailInput=document.querySelector('#checkoutForm [name="email"]'); if(emailInput && user?.email) emailInput.value=user.email;
    if(addrs.length){
      area.innerHTML=addrs.map((a,i)=>`<div class="saved-address-card" data-index="${i}"><div class="saved-address-head"><input type="radio" name="savedAddressChoice" onchange="selectSavedAddress(${i})"><div onclick="toggleSavedAddressDetails(${i})"><div class="saved-address-title">${a.label||'Address'}</div><div class="saved-address-preview">${[a.city,a.street].filter(Boolean).join(' · ')||'Saved delivery address'}</div></div></div><div class="saved-address-details">${[a.street,a.building&&('Building '+a.building),a.floor&&('Floor '+a.floor),a.apartment&&('Apartment '+a.apartment),a.city,a.phone&&('Phone '+a.phone),a.landmark&&('Landmark '+a.landmark),a.preferredTime&&('Preferred time '+a.preferredTime)].filter(Boolean).join('<br>')}</div></div>`).join('');
      formWrap.classList.add('collapsed');
      setTimeout(()=>selectSavedAddress(0),20);
    }else{
      area.innerHTML=''; formWrap.classList.remove('collapsed');
    }
  };
  function cartSubtotal(){const ps=(typeof getProducts==='function'?getProducts():[]); return readJSON('nitaCart',[]).reduce((s,i)=>{const p=ps.find(x=>x.id===i.id);return s+Number(p?.salePrice||p?.price||0)*Number(i.qty||1)},0);}
  function couponResult(subtotal){
    const form=document.getElementById('checkoutForm'); const code=String(form?.elements.coupon?.value||'').trim().toUpperCase(); const email=normEmail(form?.elements.email?.value||currentEmail());
    if(typeof evaluateCoupon==='function') return evaluateCoupon(code,subtotal,email);
    return {discount:0,message:''};
  }
  window.renderCheckoutSummary=function(){
    const box=document.getElementById('checkoutSummary'); if(!box)return;
    const cart=readJSON('nitaCart',[]); const ps=(typeof getProducts==='function'?getProducts():[]);
    if(!cart.length){box.innerHTML='<div class="empty-checkout-message">Your cart is empty.</div><div class="summary-line"><span>Subtotal</span><b>$0.00</b></div><div class="summary-line"><span>Shipping</span><b>$0.00</b></div><div class="summary-line summary-total"><span>Total</span><b>$0.00</b></div>'; const btn=document.getElementById('placeOrderBtn'); if(btn){btn.disabled=true;btn.classList.add('disabled');}return;}
    const subtotal=cartSubtotal(); const result=couponResult(subtotal); const discount=Number(result.discount||0); const total=Math.max(0,subtotal-discount)+DELIVERY_FEE;
    const items=cart.map(i=>{const p=ps.find(x=>x.id===i.id); const unit=Number(p?.salePrice||p?.price||0); const img=(typeof productMainImage==='function'&&p)?productMainImage(p):''; const bg=img.startsWith('data:')?`background-image:url(${img})`:`background:${img||'#eee'}`; return `<div class="summary-line checkout-item-line"><span><span class="checkout-thumb" style="${bg};background-size:cover;background-position:center"></span><span><b>${p?.name||'Product'}</b><br><span class="muted">${i.size||''} × ${i.qty||1}</span></span></span><b>${money(unit*Number(i.qty||1))}</b></div>`}).join('');
    box.innerHTML=items+`<hr><div class="summary-line"><span>Subtotal</span><b>${money(subtotal)}</b></div>${discount?`<div class="summary-line"><span>Discount</span><b>-${money(discount)}</b></div>`:''}<div class="summary-line"><span>Shipping</span><b>${money(DELIVERY_FEE)}</b></div><div class="delivery-info-minimal"><b>Aramex delivery across Lebanon</b><span>$7 delivery fee · 2–3 business days</span></div><div class="summary-line summary-total"><span>Total</span><b>${money(total)}</b></div>`;
    const btn=document.getElementById('placeOrderBtn'); if(btn){btn.disabled=false;btn.classList.remove('disabled');}
  };
  function validateCheckoutFinal(){const form=document.getElementById('checkoutForm'); if(!form)return false; let ok=true; const need=['email','firstName','lastName','phone','city','street','building','floor','apartment']; form.querySelectorAll('.invalid').forEach(e=>e.classList.remove('invalid')); if(!readJSON('nitaCart',[]).length){toast&&toast('Your cart is empty.');return false;} need.forEach(n=>{const el=form.elements[n]; if(el && !String(el.value||'').trim()){el.classList.add('invalid'); ok=false;}}); if(!ok){toast&&toast('Please complete the required delivery details.');} return ok;}
  function collectCheckoutAddress(){const f=document.getElementById('checkoutForm'); return {label:f.elements.addressLabel?.value||'Home',firstName:f.elements.firstName?.value||'',lastName:f.elements.lastName?.value||'',phone:f.elements.phone?.value||'',city:f.elements.city?.value||'',street:f.elements.street?.value||'',building:f.elements.building?.value||'',floor:f.elements.floor?.value||'',apartment:f.elements.apartment?.value||'',postal:f.elements.postal?.value||'',landmark:f.elements.landmark?.value||'',preferredTime:f.elements.preferredTime?.value||'',notes:f.elements.notes?.value||''};}
  window.placeOrder=async function(){
    if(!validateCheckoutFinal())return; const form=document.getElementById('checkoutForm'); const products=(typeof getProducts==='function'?getProducts():[]); const subtotal=cartSubtotal(); const result=couponResult(subtotal); const discount=Number(result.discount||0); const total=Math.max(0,subtotal-discount)+DELIVERY_FEE; const address=collectCheckoutAddress(); const email=normEmail(form.elements.email.value||currentEmail());
    if(form.elements.saveAddress?.checked || currentEmail()){
      localStorage.setItem('nitaSavedAddress',JSON.stringify(address));
      if(currentEmail()){
        const user=userRecord()||{email}; const addresses=(user.addresses||[]).filter(a=>JSON.stringify(cleanAddr(a,0))!==JSON.stringify(cleanAddr(address,0))); addresses.unshift(address);
        setUserRecord({...user,email,firstName:form.elements.firstName.value,lastName:form.elements.lastName.value,phone:form.elements.phone.value,defaultAddress:address,addresses:addresses.slice(0,6)});
      }
    }
    const order={id:'NS'+Date.now(),date:new Date().toLocaleString(),customer:(form.elements.firstName.value+' '+form.elements.lastName.value).trim(),phone:form.elements.phone.value,email,address,payment:'Cash on Delivery',status:'New order',items:readJSON('nitaCart',[]),subtotal,discount,shipping:DELIVERY_FEE,coupon:String(form.elements.coupon?.value||'').trim().toUpperCase(),total};
    const orders=readJSON('nitaOrders',[]); orders.push(order); writeJSON('nitaOrders',orders); if(typeof saveSharedKeyNow==='function') await saveSharedKeyNow('nitaOrders',orders).catch(()=>{});
    localStorage.setItem('nitaCart','[]'); if(typeof saveCart==='function'){cart=[]; saveCart();}
    try{if(typeof sendStoreEmail==='function') await sendStoreEmail({type:'order_confirmation',to:email,order,products});}catch(e){}
    location.href='order-success.html';
  };
  window.applyCouponCode=function(){window.renderCheckoutSummary(); const fb=document.getElementById('couponFeedback'); if(fb){const subtotal=cartSubtotal(); const res=couponResult(subtotal); fb.innerHTML=res.message||'<span class="valid">Coupon applied.</span>';}}
  function bootCheckout(){if(!document.getElementById('checkoutForm'))return; setupCheckoutAddresses(); renderCheckoutSummary(); document.getElementById('checkoutForm').addEventListener('input',()=>setTimeout(renderCheckoutSummary,0));}
  document.addEventListener('DOMContentLoaded',bootCheckout); window.addEventListener('load',bootCheckout); window.addEventListener('pageshow',bootCheckout); setTimeout(bootCheckout,400);
})();
/* === END PROFESSIONAL CHECKOUT FINAL PATCH === */
