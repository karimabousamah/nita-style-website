...
(function(){
  ...
  function revealAfterIntro(){
    ...
    const onHome=/\/index\.html?$|\/$/.test(location.pathname) || location.pathname==='' || location.pathname.endsWith('/');
    const intro=()=>$('#introLoader');
    if(!onHome){ setTimeout(show,450); return; }
    const startWatching=()=>{
      if(!intro()){
        setTimeout(()=>{ if(!intro()) show(); },2300);
        return;
      }
      const obs=new MutationObserver(()=>{ if(!intro()){ obs.disconnect(); setTimeout(show,300); }});
      obs.observe(document.body,{childList:true,subtree:true});
      setTimeout(()=>{ if(!intro()) show(); },4200);
    };
    if(document.readyState==='complete') startWatching();
    else window.addEventListener('load',()=>setTimeout(startWatching,80),{once:true});
  }
  ...
  function suggestions(){
    ...
    body().insertAdjacentHTML('beforeend',`<div class="nita-ai-suggestion-box" id="nitaAiSuggestionsBox"><button type="button" class="nita-ai-suggestion-toggle"><span>Suggested questions</span><small>tap to open</small><span class="mark">+</span></button><div class="nita-ai-suggestions">${html}</div></div>`);
    const box=$('#nitaAiSuggestionsBox');
    const toggle=box.querySelector('.nita-ai-suggestion-toggle');
    toggle.onclick=()=>{box.classList.toggle('open');toggle.querySelector('small').textContent=box.classList.contains('open')?'tap to close':'tap to open';toggle.querySelector('.mark').textContent=box.classList.contains('open')?'−':'+'};
  ...
  function ask(text){msg(text,'user'); typing(); setTimeout(()=>{untyping(); answer(text)},260)}

  function answer(raw){
    const q=raw.toLowerCase();
    const budget=getBudget(q);
    if(/track|order|status|roadmap/.test(q)&&!/shipping|deliver/.test(q))return orderHelp();
    if(/ship|deliver|delivery|arrive|how long|how many day|how much day|time/.test(q))return bot('Delivery usually takes 2 to 4 working days after the order is confirmed. At checkout, enter your city, street, building/floor, phone number, and any delivery notes. You can then follow the order roadmap from your account.');
    if(/most expensive|highest price|priciest|luxury/.test(q)) return productHelp(q,{sort:'expensive'});
    if(/cheapest|lowest price|least expensive/.test(q)) return productHelp(q,{sort:'cheap'});
    ...
    if(/return|refund|exchange/.test(q))return bot('For returns or exchanges, contact Nita Style as soon as possible with your order number and item details. The team will review the request based on item condition and order information.');
  ...
  function orderHelp(){
    const u=user();
    if(!u?.email){bot('To follow your order, please sign in with the same email used at checkout. Then open Account to see the roadmap.'); return;}
  ...
  function productHelp(q,opts={}){
    ...
    const words=q.split(/[^a-z0-9]+/).filter(w=>w.length>2&&!['product','search','find','shop','price','collection','want','with','what','recommend','suggest','budget','under','less','than','gift','present','show','most','expensive'].includes(w));
    ...
    if(opts.sort==='expensive') list=list.sort((a,b)=>Number(b.p.salePrice||b.p.price||0)-Number(a.p.salePrice||a.p.price||0));
    else if(opts.sort==='cheap') list=list.sort((a,b)=>Number(a.p.salePrice||a.p.price||0)-Number(b.p.salePrice||b.p.price||0));
    else list=list.sort((a,b)=>b.score-a.score || Number(b.p.salePrice||b.p.price||0)-Number(a.p.salePrice||a.p.price||0));
    list=list.slice(0,opts.sort?3:4).map(x=>x.p);
    if(!list.length && opts.budget){
      const cheapest=ps.slice().sort((a,b)=>Number(a.salePrice||a.price||0)-Number(b.salePrice||b.price||0)).slice(0,4);
  ...
  function renderProducts(list){
    list.forEach(p=>{
      ...
      const mediaHtml=media.type==='img'?`<div class="nita-ai-product-media"><img src="${esc(media.src)}" alt="${esc(p.name||'Nita Style item')}"></div>`:media.type==='bg'?`<div class="nita-ai-product-media no-photo" style="background:${esc(media.src)}"></div>`:`<div class="nita-ai-product-media no-photo"><span>Nita Style</span></div>`;
      body().insertAdjacentHTML('beforeend',`<a class="nita-ai-product" href="${url}">${mediaHtml}<div class="nita-ai-product-info"><div class="nita-ai-product-top"><strong>${esc(p.name||'Nita Style item')}</strong><span>${money(p.salePrice||p.price)}</span></div><div class="nita-ai-product-meta">${esc(p.category||'Collection')} · ${esc(p.collection||'Nita Style')}</div><p>${esc(p.desc||'Selected Nita Style piece.')}</p><span class="nita-ai-product-cta">Open product</span></div></a>`)
    });
    scroll();
  }
